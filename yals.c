...
#define INIT_DDFW_WEIGHT 8.0 // Only applies to wtrule 4 and 5 #define INIT_DDFW_WEIGHT 100 and INVWT_CONSTANT 8.0
#define INVWT_CONSTANT 8.0
...
void yals_reset_ddfw (Yals * yals)
{
  ...
  //   yals->ddfw.clause_weights [i] = INIT_DDFW_WEIGHT;
}
...
{
  double w;
  if (yals->ddfw.clause_weights[source] > INIT_DDFW_WEIGHT)
    w = 2.0f;
  else
    w = 1.0f;
  return w;
...
double lmbased_wt (Yals * yals, int source, int sink)
{
  ...
  double defwt = 1.0f;//(yals->ddfw.clause_weights [source] > INIT_DDFW_WEIGHT ? 1.0f : 2.0f);
  ...
  if (shortfall_determined && yals->ddfw.guaranteed_uwrvs < 1)
  {
    ...
    if (yals->ddfw.clause_weights [source] - decby >= (INIT_DDFW_WEIGHT - 1.0))
    {
      w = decby;
...
double linear_wt (Yals * yals, int source, int sink)
{
  ...
  {
    a = yals->ddfw.clause_weights [source] > INIT_DDFW_WEIGHT ? (float) yals->opts.paramAbig.val / 100.0 : (float) yals->opts.paramAsmall.val / 100.0;
    c = yals->ddfw.clause_weights [source] > INIT_DDFW_WEIGHT ? yals->opts.paramCBig.val : yals->opts.paramCSmall.val;
  }
  else if (yals->opts.wtrulelinchoice.val == 2) // lw-ith
  {
    a = yals->ddfw.clause_weights [source] > INIT_DDFW_WEIGHT ? (float) yals->opts.paramAsmall.val / 100.0 : (float) yals->opts.paramAbig.val / 100.0;
    c = yals->ddfw.clause_weights [source] > INIT_DDFW_WEIGHT ? yals->opts.paramCSmall.val : yals->opts.paramCBig.val;
  }
  ...
  w = (double) ((float) yals->ddfw.clause_weights [source] * (float) a +  (float) c);
  return w;
}
...
void yals_ddfw_transfer_weights_for_clause (Yals *yals, int sink)
{
  int * lits = yals_lits (yals, sink);
  int lit;
  double best_w = !yals->opts.ignorewtcriteria.val ? INIT_DDFW_WEIGHT : 0;
  int source = -1;
  ...
  if ( source == -1  || ( ( (double) yals_rand_mod (yals, INT_MAX) / (double) INT_MAX) <= yals->ddfw.clsselectp))
  {
    ...
    while (source<0) 
    {
       ...
       {
         if (yals->ddfw.clause_weights [clause] >= INIT_DDFW_WEIGHT) 
            source = clause;
       }
    }
  }
